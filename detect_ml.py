    ...
    while True:
        ...
        for i, keypoints in enumerate(keypoints_data):
            if len(keypoints) > 0 and i < len(boxes):
                ...
                # Trích xuất đặc trưng giống lúc tạo dataset
                normalized_kpts = []
                for kp in keypoints:
                    kx, ky = kp[0], kp[1]
                    kx_norm = (kx - x1) / w
                ...
                # Dự đoán trạng thái bằng SVM
                pred = clf.predict(features_scaled)[0]
                pred_proba = clf.predict_proba(features_scaled)[0]
                
                # Nhãn 1 là ngã, 0 là bình thường
                status = 'Fallen' if pred == 1 else 'Standing'
                confidence = pred_proba[pred] # Lấy độ tin cậy từ xác suất dự đoán của SVM
